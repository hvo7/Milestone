...
interface QuestData {
  ...
  /** Pin (or unpin) every visible task in a quest to the Today list at once. */
  setQuestActionsTracked: (qlId: string, qId: string, tracked: boolean) => void;
  ...
  persist(
    (set) => ({
      ...
      toggleAction: (qlId, qId, aId) =>
        set(s => {
          const was = s.questlines.find(ql => ql.id === qlId)?.quests.find(q => q.id === qId)?.actions.find(a => a.id === aId)?.completed ?? false;
          return {
            questlines: s.questlines.map(ql => ql.id !== qlId ? ql : { ...ql, quests: ql.quests.map(q => q.id !== qId ? q : { ...q, actions: q.actions.map(a => a.id !== aId ? a : { ...a, completed: !a.completed }) }) }),
            completionLog: bumpLog(s.completionLog, was ? -1 : 1),
          };
        }),

      toggleTracked: (qlId, qId, aId) =>
        set(s => ({ questlines: s.questlines.map(ql => ql.id !== qlId ? ql : { ...ql, quests: ql.quests.map(q => q.id !== qId ? q : { ...q, actions: q.actions.map(a => a.id !== aId ? a : { ...a, trackedToday: !a.trackedToday }) }) }) })),

      // Hidden tasks are left alone: they're deliberately out of play, and pinning
      // one would drop a task the user hid straight back onto Today.
      setQuestActionsTracked: (qlId, qId, tracked) =>
        set(s => ({ questlines: s.questlines.map(ql => ql.id !== qlId ? ql : { ...ql, quests: ql.quests.map(q => q.id !== qId ? q : { ...q, actions: q.actions.map(a => a.hidden ? a : { ...a, trackedToday: tracked }) }) }) })),

      toggleQuestTracked: (qlId, qId) =>
        set(s => ({ questlines: s.questlines.map(ql => ql.id !== qlId ? ql : { ...ql, quests: ql.quests.map(q => q.id !== qId ? q : { ...q, trackedToday: !q.trackedToday }) }) })),
      ...
      setQuestComplete: (qlId, qId, complete) =>
        set(s => {
          const quest = s.questlines.find(ql => ql.id === qlId)?.quests.find(q => q.id === qId);
          if (!quest) return {};
          const visible = quest.actions.filter(a => !a.hidden);
          let delta: number;
          let nextQuest: Quest;
          ...
          return {
            questlines: s.questlines.map(ql => ql.id !== qlId ? ql : { ...ql, quests: ql.quests.map(q => q.id !== qId ? q : nextQuest) }),
            completionLog: delta === 0 ? s.completionLog : bumpLog(s.completionLog, delta),
          };
        }),

      toggleActionHidden: (qlId, qId, aId) =>
        set(s => ({ questlines: s.questlines.map(ql => ql.id !== qlId ? ql : { ...ql, quests: ql.quests.map(q => q.id !== qId ? q : { ...q, actions: q.actions.map(a => a.id !== aId ? a : { ...a, hidden: !a.hidden }) }) }) })),

      setActionRecurring: (qlId, qId, aId, r, intervalDays, monthlyRule) =>
        set(s => ({ questlines: s.questlines.map(ql => ql.id !== qlId ? ql : { ...ql, quests: ql.quests.map(q => q.id !== qId ? q : { ...q, actions: q.actions.map((a): Action => {
          if (a.id !== aId) return a;
          const nextRecurring = r ?? null;
          const nextInterval = intervalDays || undefined;
          const nextRule = monthlyRule ?? null;
          // The RepeatPicker re-emits on any click, so re-selecting the current cadence
          // must be a no-op — otherwise it would restart the reset clock and re-pin it.
          const changed = nextRecurring !== (a.recurring ?? null)
            || nextInterval !== (a.intervalDays || undefined)
            || !sameRule(nextRule, a.monthlyRule);
          if (!changed) return a;
          return {
            ...a,
            ...
            trackedToday: nextRecurring === 'daily' && !nextInterval && !nextRule ? true : a.trackedToday,
            lastResetAt: (nextRecurring || nextInterval || nextRule) ? new Date().toISOString() : undefined,
          };
        }) }) }) })),

      toggleQuestHidden: (qlId, qId) =>
        set(s => ({ questlines: s.questlines.map(ql => ql.id !== qlId ? ql : { ...ql, quests: ql.quests.map(q => q.id !== qId ? q : { ...q, hidden: !q.hidden }) }) })),

      setQuestRecurring: (qlId, qId, r) =>
        set(s => ({ questlines: s.questlines.map(ql => ql.id !== qlId ? ql : { ...ql, quests: ql.quests.map(q => q.id !== qId ? q : { ...q, recurring: r ?? null, lastResetAt: r ? new Date().toISOString() : undefined, streak: r ? (q.streak ?? 0) : undefined }) }) })),

      setQuestDueDate: (qlId, qId, dueDate) =>
        set(s => ({ questlines: s.questlines.map(ql => ql.id !== qlId ? ql : { ...ql, quests: ql.quests.map(q => q.id !== qId ? q : { ...q, dueDate: dueDate ?? null }) }) })),

      addAction: (qlId, qId, title) =>
        set(s => ({ questlines: s.questlines.map(ql => ql.id !== qlId ? ql : { ...ql, quests: ql.quests.map(q => q.id !== qId ? q : { ...q, actions: [...q.actions, { id: `a-${uid()}`, title, completed: false, trackedToday: false }] }) }) })),

      deleteAction: (qlId, qId, aId) =>
        set(s => ({ questlines: s.questlines.map(ql => ql.id !== qlId ? ql : { ...ql, quests: ql.quests.map(q => q.id !== qId ? q : { ...q, actions: q.actions.filter(a => a.id !== aId) }) }) })),

      toggleQuestlineHidden: (qlId) =>
        set(s => ({ questlines: s.questlines.map(ql => ql.id !== qlId ? ql : { ...ql, hidden: !ql.hidden }) })),
      ...
      addQuest: (qlId, title, desc, recurring, dueDate, monthlyRule) =>
        set(s => ({
          questlines: s.questlines.map(ql => {
            if (ql.id !== qlId) return ql;
            const order = ql.quests.length + 1;
            const q: Quest = { id: `q-${uid()}`, title, description: desc, order, actions: [], recurring: recurring ?? null, ...(monthlyRule ? { monthlyRule } : {}), lastResetAt: (recurring || monthlyRule) ? new Date().toISOString() : undefined, streak: 0, dueDate: monthlyRule ? null : (dueDate ?? null) };
            return { ...ql, quests: [...ql.quests, q] };
          }),
        })),

      updateQuestTitle: (qlId, qId, title) =>
        set(s => ({ questlines: s.questlines.map(ql => ql.id !== qlId ? ql : { ...ql, quests: ql.quests.map(q => q.id !== qId ? q : { ...q, title }) }) })),
      ...
      updateQuest: (qlId, qId, updates) =>
        set(s => ({
          questlines: s.questlines.map(ql => ql.id !== qlId ? ql : {
            ...ql,
            quests: ql.quests.map(q => {
              ...
              const scheduleChanged = nextRecurring !== (q.recurring ?? null) || !sameRule(nextRule, q.monthlyRule);
              ...
              return {
                ...
                lastResetAt: scheduleChanged ? (stillRepeats ? new Date().toISOString() : undefined) : q.lastResetAt,
                streak: scheduleChanged ? (stillRepeats ? (q.streak ?? 0) : undefined) : q.streak,
              };
            }),
          }),
        })),
      ...
      deleteQuest: (qlId, qId) =>
        set(s => ({
          questlines: s.questlines.map(ql => ql.id !== qlId ? ql : { ...ql, quests: ql.quests.filter(q => q.id !== qId).map((q, i) => ({ ...q, order: i + 1 })) }),
      ...
      reorderQuests: (qlId, orderedIds) =>
        set(s => ({
          questlines: s.questlines.map(ql => ql.id !== qlId ? ql : {
            ...ql,
            quests: orderedIds.map((id, i) => {
              const q = ql.quests.find(q => q.id === id)!;
              return { ...q, order: i + 1 };
            }),
          }),
        })),
      ...
      setRoutineRecurring: (rId, recurring, intervalDays, monthlyRule) =>
        set(s => ({ routines: s.routines.map(r => {
          if (r.id !== rId) return r;
          const nextInterval = intervalDays || undefined;
          const nextRule = monthlyRule ?? null;
          // The RepeatPicker re-emits on any click, so re-picking the current cadence
          // must be a no-op — otherwise it would wipe the streak and restart the clock.
          const changed = (recurring ?? null) !== (r.recurring ?? null)
            || nextInterval !== (r.intervalDays || undefined)
            || !sameRule(nextRule, r.monthlyRule);
          if (!changed) return r;
          return {
            ...r,
            recurring,
            intervalDays: nextInterval,
            monthlyRule: nextRule,
            trackedToday: recurring === 'daily' && !nextInterval && !nextRule ? true : r.trackedToday,
      ...
      // Assigns sequential order values to the given ids (a single Today group).
      // Other routines keep their order; each Today group is sorted independently,
      // so values colliding across groups is harmless.
      reorderRoutines: (orderedIds) =>
        set(s => {
          const pos = new Map(orderedIds.map((id, i) => [id, i]));
          return { routines: s.routines.map(r => pos.has(r.id) ? { ...r, order: pos.get(r.id)! } : r) };
        }),

      setRoutineQuest: (rId, questId) =>
        set(s => ({ routines: s.routines.map(r => r.id !== rId ? r : { ...r, questId: questId ?? undefined }) })),
      ...
      renameRoutineSubtask: (rId, sId, title) =>
        set(s => ({ routines: s.routines.map(r => r.id !== rId ? r : { ...r, subtasks: mapNode(r.subtasks ?? [], sId, st => ({ ...st, title })) }) })),
        ...
        set(s => ({
          routines: s.routines.map(r => {
            ...
            if (u.recurring !== undefined || u.monthlyRule !== undefined) {
              const nextInterval = u.intervalDays || undefined;
              const nextRule = u.monthlyRule !== undefined ? (u.monthlyRule ?? null) : (r.monthlyRule ?? null);
              const nextRecurring = u.recurring !== undefined ? u.recurring : (r.recurring ?? null);
              const changed = nextRecurring !== (r.recurring ?? null)
                || nextInterval !== (r.intervalDays || undefined)
                || !sameRule(nextRule, r.monthlyRule);
              if (changed) {
                next.recurring    = nextRecurring;
                next.intervalDays = nextInterval;
                next.monthlyRule  = nextRule;
                next.trackedToday = nextRecurring === 'daily' && !nextInterval && !nextRule ? true : r.trackedToday;
                next.lastResetAt  = new Date().toISOString();
                next.streak       = r.streak ?? 0;
                if (nextRecurring || nextRule) next.dueDate = null;
              }
            }
            return next;
          }),
      ...
      toggleRoutineTracked: (rId) =>
        set(s => ({ routines: s.routines.map(r => r.id !== rId ? r : { ...r, trackedToday: !r.trackedToday }) })),

      toggleRoutineHidden: (rId) =>
        set(s => ({ routines: s.routines.map(r => r.id !== rId ? r : { ...r, hidden: !r.hidden }) })),

      updateRoutineTitle: (rId, title) =>
        set(s => ({ routines: s.routines.map(r => r.id !== rId ? r : { ...r, title }) })),

      updateActionTitle: (qlId, qId, aId, title) =>
        set(s => ({ questlines: s.questlines.map(ql => ql.id !== qlId ? ql : { ...ql, quests: ql.quests.map(q => q.id !== qId ? q : { ...q, actions: q.actions.map(a => a.id !== aId ? a : { ...a, title }) }) }) })),
      ...
      setRoutineCompleted: (rId, completed) =>
        set(s => ({ routines: s.routines.map(r => r.id !== rId ? r : { ...r, completed, completedAt: completed ? (r.completedAt ?? new Date().toISOString()) : undefined }) })),

      setAllActionsComplete: (qlId, qId, complete) =>
        set(s => ({ questlines: s.questlines.map(ql => ql.id !== qlId ? ql : { ...ql, quests: ql.quests.map(q => q.id !== qId ? q : { ...q, actions: q.actions.map(a => ({ ...a, completed: complete })) }) }) })),
    }),
