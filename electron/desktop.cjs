 ...
 */
function migrateLegacyProfile(legacyPath, target) {
  const LEGACY_USER_DATA = legacyPath;
...
/** Register once at startup. Resolve the current window at invocation time. */
function registerIpcHandlers({ ipcMain, shell, notion, cloudSync, phone, updater, backups, tray, getWindow }) {
  const { loadConfig, saveConfig, testConnection, syncToNotion, pullFromNotion } = notion;
// ── Notion IPC handlers ───────────────────────────────────────────────────────
